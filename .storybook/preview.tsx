...
const preview: Preview = {
  parameters: {
    ...
    // только у «Атомов». Без явного storySort Storybook раскладывает истории
    // в порядке импорта файлов (т.е. по пути на диске), а не по заголовку,
    // поэтому алфавит нужно задать явно.
    //
    // «Атомы» (дизайн-чек от 13.09, замечание 13 — прежний рут PREVIEW) —
    // не компоненты, а материал, из которого они сделаны: палитра,
    // типографика, набор иконок и сетка.
    //
    // «Песочница» — третья группа: целые экраны продукта из секции «Песочные
    // экраны» (нода 70371:24466). Они не компоненты, а доказательство того,
    // что компоненты складываются друг с другом, поэтому и лежат отдельно, а
    // не вперемешку с витринами.
    options: {
      storySort: {
        method: 'alphabetical',
        order: ['Атомы', 'Компоненты', 'Песочница'],
      },
    },
  },
